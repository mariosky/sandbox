...
def run_test(code, test, type=None):
    try:
        ...
        #COMPILE Test
        try:
            out = subprocess.check_output(['javac','-cp','%s:/usr/share/java/hamcrest.jar:/usr/share/java/junit.jar' % tmp_dir, os.path.join(tmp_dir, "%sTest.java" % java_class)], stderr=subprocess.STDOUT)
            result = (out,0)
        except subprocess.CalledProcessError , e:
            result = (json.dumps({ 'successes':[],'failures':[], 'errors': e.output.split('\n'), 'stdout': "", 'result': "Failure"}),e.returncode)
            return result
        ...
        #TEST
        try:
            out = subprocess.check_output(['java','-cp', '%s:/usr/share/java/hamcrest.jar:/usr/share/java/junit.jar' % tmp_dir ,'org.junit.runner.JUnitCore', "%sTest" % java_class], stderr=subprocess.STDOUT)
